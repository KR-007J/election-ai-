    ...
    <div className="space-y-8 max-w-screen-container-max mx-auto">
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-3 flex lg:flex-col gap-2 overflow-x-auto pb-4 lg:pb-0 custom-scrollbar">
          <button type="button" className="flex items-center gap-3 px-6 py-4 rounded-xl glass-panel text-white border-primary/20 whitespace-nowrap bg-white/10 ring-1 ring-primary/30">
            <AppIcon name="notifications" className="h-5 w-5 text-primary" />
            <span className="font-h3 text-[18px]">Notifications</span>
          </button>
          {tabs.map((tab) => (
            <button type="button" key={tab.label} className="flex items-center gap-3 px-6 py-4 rounded-xl glass-panel text-slate-400 hover:text-white transition-colors whitespace-nowrap border border-white/5">
              <AppIcon name={tab.icon} className="h-5 w-5" />
              <span className="font-h3 text-[18px]">{tab.label}</span>
            </button>
          ))}
        </div>
        ...
        <div className="col-span-12 lg:col-span-9 space-y-md">
          ...
          >
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center">
                <AppIcon name="verified" className="h-8 w-8 text-primary" />
              </div>
              <div>
            ...
            <button type="button" className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-xs font-bold text-white hover:bg-white/10 transition-all">
          ...
          >
            <div className="mb-lg">
              <h3 className="font-h3 text-h3 text-white mb-1">Visual Accessibility</h3>
              <p className="text-sm text-slate-500">Enhance readability for better interaction.</p>
            </div>

            <div className="space-y-4">
              <div className="p-6 rounded-2xl bg-white/5 border border-white/5 hover:border-primary/30 transition-all flex justify-between items-center group">
                <div className="max-w-xl">
                  <h4 className="text-white font-bold mb-1 group-hover:text-primary transition-colors">High Contrast Mode</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">Increase contrast between text and background for maximum legibility.</p>
                </div>
                <button 
                  type="button"
                  onClick={toggleHighContrast}
                  aria-pressed={isHighContrast}
                  className={`w-12 h-6 rounded-full relative transition-all cursor-pointer ${isHighContrast ? "bg-primary" : "bg-slate-700"}`}
                >
                  <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-md ${isHighContrast ? "right-1" : "left-1"}`}></div>
                </button>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              {["Education", "Healthcare", "Economy", "Climate", "Technology", "Infrastructure", "Human Rights"].map(issue => (
                <button type="button" key={issue} className="px-5 py-2 rounded-full border border-white/10 hover:border-primary/50 text-xs font-bold text-slate-400 hover:text-primary transition-all">
                  {issue}
                </button>
              ))}
            </div>
      ...
      <div className="flex items-center justify-end gap-6 pt-6 border-t border-white/5">
        ...
        <button type="button" className="text-slate-400 hover:text-white transition-colors font-bold px-6 py-2">
          Discard Changes
        </button>
        <button 
          type="button"
